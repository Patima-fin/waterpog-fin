...
//        - จ่ายแล้ว  = pvVouchers (รหัสโครงการฝังใน Project_Dpt / col A)
//        - ค้างจ่าย  = payables   (รหัสโครงการฝังใน pre_des / col AF)
//     แยกงวดด้วย ADV / งวด 1 / งวด 2 … จาก remark
// จับคู่รายงวด (ตามที่ผู้ใช้เลือก): ADV ← เซ็นสัญญา · งวด N ← รับเงิน AR งวด N
//   → จ่ายแล้ว / พร้อมจ่าย / เกินกำหนด / รอเงื่อนไข / วางแผนแล้ว
// ปุ่ม "วางแผนจ่าย" เขียน forecastEntries (EXPENSE_TYPE='AP', REF_DOC=vchno,
//   STATUS='PLANNED') แบบเดียวกับ Bank Diary → โผล่หน้า Bank Daily.
//
// ★ รหัสโครงการของ PV (Project_Dpt) sync เก็บไว้แล้ว — แกะใช้ได้เลย.
//   ของ payables (pre_des) ต้อง re-import ไฟล์ AP ที่มี col AF (ระบบ import เก็บ
//   ทุกคอลัมน์อยู่แล้ว — Supabase JSONB) แล้วจะผูกโครงการได้.
//
// identifiers ทุกตัว prefix PR/pr กัน global-scope collision.
// ═══════════════════════════════════════════════════════════════════════════
...
//   "Project : (00102026) ENC165-PL-บ้าน…" → ENC165
//   "TTI043-PDH-บ้านเขาแดง…"               → TTI043
//   "Advance 40%(Project : STR072)"        → STR072  · "Advance 40%(PP068)" → PP068
function prCodeFromText(text) {
  const s = String(text == null ? '' : text);
  let m = s.match(/Project\s*:\s*\(\d+\)\s*([A-Za-z]{2,5}\d{2,4})/);   // PV Project_Dpt
  if (m) return m[1].toUpperCase();
  m = s.match(/\(\s*(?:Project\s*:\s*)?([A-Za-z]{2,5}\d{2,4})\b/);     // "(PP068)" / "(Project : STR072)"
  if (m) return m[1].toUpperCase();
  m = s.match(/^\s*([A-Za-z]{2,5}\d{2,4})\s*-/);                       // pre_des starts with code
  if (m) return m[1].toUpperCase();
  m = s.match(/\b([A-Za-z]{2,5}\d{2,4})\b/);                          // any code-like token
  if (m) return m[1].toUpperCase();
  return '';
}
...
function prBuildAll(data) {
  ...
  // ลูกหนี้ (customer) จากใบแจ้งหนี้
  const custByCode = {};
  invoices.forEach(iv => {
  ...
  // ── AP PAID: pvVouchers (APS) → group by project code (จาก Project_Dpt) ──
  const paidByCode = {};
  ...
  pvs.forEach(v => {
    ...
    (paidByCode[code] = paidByCode[code] || []).push({
      vendor: v.Payee || v.cust_name || '—',
      pvNo: v.PL_PV_No || '', apNo: apno,
      amount: prToNum(v.Net_Amount),
      date: prISO(v.Pmt_Date),
      inst: prInst(v.Remark || v.cc_remark),
    });
  });

  // ── AP OUTSTANDING: payables (APS) → group by project code (จาก pre_des) ──
  const outByCode = {};
  payables.forEach(p => {
    ...
    if (vchno && paidVchno.has(vchno)) return;   // จ่ายผ่าน PV แล้ว — ไม่นับค้าง
  ...
  // forecast ที่ผูก AP (REF_DOC) ที่ยังเป็นแผน
  const fcByRef = {};
  forecasts.forEach(f => {
  ...
  const projects = derived
    ...
    .map(d => {
      ...
      const recvByNo = {};   // งวด no → receivedDate (สำหรับ gate)
      const arItems = arRaw.map(it => {
        const inv = it.invoice || {};
        const recDate = inv.receivedDate || null;
        if (recDate) recvByNo[it.no] = recDate;
        return {
          no: it.no, label: singleAr ? 'ครั้งเดียว' : ('งวด ' + it.no),
          invoiceNo: inv.ivNo || '—', amount: it.amount || prToNum(inv.balance),
          receivedDate: recDate, received: !!recDate,
        };
      });
      const signedDate = d.start || null;

      // ── AP groups ราย "งวด" (ADV/งวด N/อื่นๆ) รวม จ่ายแล้ว(PV) + ค้างจ่าย(payable) ──
      const gmap = {};
      ...
      const groups = Object.values(gmap).map(g => {
        // gate ราย งวด: ADV ← เซ็นสัญญา · งวด N ← รับ AR งวด N · อื่นๆ/คืนประกัน ← ไม่ gate
        let condLabel = '—', unlocked = true, unlockDate = signedDate;
        if (g.key === 'ADV') { condLabel = 'เซ็นสัญญา'; unlocked = !!signedDate || true; unlockDate = signedDate; }
        else { const mm = g.key.match(/งวด\s*(\d+)/);
          if (mm) { const n = +mm[1]; condLabel = 'รับ AR งวด ' + n; unlockDate = recvByNo[n] || null; unlocked = !!recvByNo[n]; }
        }
        // สถานะกลุ่ม
      ...
      // ผู้รับเหมาหลัก = ผู้รับเงินยอดรวมมากสุด (จาก paid+out)
      const byVendor = {};
...
function PaymentReconPage({ data, setData, toast }) {
  ...
  const [planTarget, setPlanTarget] = React.useState(null);   // {proj, group}
  ...
  const agg = React.useMemo(() => {
    ...
    fyProjects.forEach(p => p.apGroups.forEach(g => {
      ...
      if (g.paidAmt > 0) counts.paid++;
      if (g.outAmt > 0 && totals[g.status] != null) { totals[g.status] += g.outAmt; counts[g.status]++; }
    }));
  ...
  // ── เขียน: วางแผนจ่าย (ค้างจ่ายทั้งงวด) → forecastEntries (รูปแบบ Bank Diary) ──
  const doPlan = () => {
  ...
  const kpiCards = [
    ...
    { key: 'waiting', desc: 'ยังไม่รับ AR งวดนั้น', accent: '#f59e0b', val: agg.totals.waiting, cnt: agg.counts.waiting, color: '#b45309' },
    ...
    { key: 'paid', desc: 'จ่ายผู้รับเหมาแล้ว (PV)', accent: '#94a3b8', val: agg.totals.paid, cnt: agg.counts.paid, color: '#475569' },
  ];
  ...
  return (
    <div className="pr-page" style={{ maxWidth: 1240, margin: '0 auto', padding: '4px 2px 60px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 24, flexWrap: 'wrap', marginBottom: 22 }}>
        <div>
          ...
          <p style={{ margin: '6px 0 0', fontSize: 13.5, color: 'var(--ink-500)', maxWidth: 620, lineHeight: 1.5 }}>
            จับคู่เงินรับจากลูกหนี้ <b style={{ color: 'var(--ink-700)' }}>(AR)</b> กับการจ่ายผู้รับเหมา <b style={{ color: 'var(--ink-700)' }}>(AP)</b> รายงวด — จ่ายผู้รับเหมาเมื่อรับเงิน AR งวดนั้นแล้ว
          </p>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', background: 'var(--brand-900)', color: '#dbe6f7', borderRadius: 12, padding: '12px 18px', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}><span style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 600, color: '#fff' }}>งวด 1</span><span style={{ color: '#5b7099' }}>←</span><span>รับเงิน AR งวด 1</span></div>
        <span style={{ color: '#3a527a' }}>·</span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13 }}><span style={{ fontFamily: "'IBM Plex Mono',monospace", fontWeight: 600, color: '#fff' }}>งวด 2</span><span style={{ color: '#5b7099' }}>←</span><span>รับเงิน AR งวด 2</span></div>
      </div>
      ...
      {fyProjects.length > 0 && !fyHasAp && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--warn-bg)', border: '1px solid #fde68a', borderRadius: 12, padding: '12px 16px', marginBottom: 14, fontSize: 13, color: '#92660b', lineHeight: 1.5 }}>
          ...
          <span>มีโครงการเซ็นแล้ว แต่ยังไม่มี AP ผูกกับโครงการ — ฝั่ง <b>จ่ายแล้ว</b> มาจาก PV (มีรหัสโครงการแล้ว) · ฝั่ง <b>ค้างจ่าย</b> ต้อง <b>อัปโหลดไฟล์เจ้าหนี้ที่มีคอลัมน์ pre_des (โครงการ)</b> ที่หน้า "DATA เจ้าหนี้คงค้าง" 1 ครั้ง แล้วจะผูกโครงการให้เอง</span>
        </div>
      )}
      ...
      <div style={{ background: '#fff', border: '1px solid var(--line)', borderRadius: 14, overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '104px minmax(0,1fr) 130px 150px 132px 38px', gap: 14, alignItems: 'center', padding: '11px 18px', background: 'var(--ink-50)', borderBottom: '1px solid var(--line-soft)', fontSize: 11, fontWeight: 600, letterSpacing: 0.4, textTransform: 'uppercase', color: 'var(--ink-400)' }}>
          ...
          <span style={{ textAlign: 'center' }}>งวด AP</span>
        ...
        {projects.length === 0 && (
          <div style={{ padding: '40px 18px', textAlign: 'center', color: 'var(--ink-500)', fontSize: 14, lineHeight: 1.7 }}>
            {diag.signed === 0 ? (
              ...
              <span>{fy === 'all' ? 'ไม่พบโครงการ' : <>ปี <b>25{fy}</b> ไม่มีโครงการที่เซ็นสัญญา</>} · ลองเลือก <b>"ทุกปี"</b><br /><span style={{ fontSize: 12.5, color: 'var(--ink-400)' }}>เซ็นแล้วรวมทุกปี <b>{diag.signed}</b> โครงการ</span></span>
            ) : (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '104px minmax(0,1fr) 130px 150px 132px 38px', gap: 14, alignItems: 'center', padding: '12px 18px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 5, justifyContent: 'center', flexWrap: 'wrap' }}>
                    {minis.map((g, i) => {
                      ...
                      return <span key={i} title={g.key + ' · ' + m.label} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 30, height: 21, padding: '0 6px', borderRadius: 6, fontSize: 10.5, fontWeight: 600, fontFamily: "'IBM Plex Mono',monospace", background: m.bg, color: m.color }}>{g.key === 'ADV' ? 'Adv' : g.key.replace('งวด ', 'ง')}</span>;
                    })}
              ...
              {expanded && (
                <div style={{ padding: '6px 18px 22px', background: 'var(--ink-50)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '10px 0 16px', fontSize: 12, color: 'var(--ink-500)', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, maxWidth: 180, height: 6, borderRadius: 999, background: 'var(--ink-200)', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.7fr', gap: 20, alignItems: 'start' }}>
                    ...
                    {/* AP — ราย งวด */}
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: 0.6, textTransform: 'uppercase', color: 'var(--ink-400)', marginBottom: 8 }}>จ่ายผู้รับเหมา · AP รายงวด ({p.apGroups.length})</div>
                      <div style={{ border: '1px solid var(--line-soft)', borderRadius: 10, overflow: 'hidden', background: '#fff' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1fr 0.85fr 0.85fr 1.05fr', background: 'var(--ink-50)', fontSize: 10.5, color: 'var(--ink-400)', padding: '8px 12px', gap: 8 }}>
                          <span>งวด</span><span>เงื่อนไข (AR)</span><span style={{ textAlign: 'right' }}>จ่ายแล้ว</span><span style={{ textAlign: 'right' }}>ค้างจ่าย</span><span style={{ textAlign: 'right' }}>สถานะ</span>
                        </div>
                        {p.apGroups.length === 0 && <div style={{ padding: '10px 12px', fontSize: 12, color: 'var(--ink-400)' }}>ยังไม่มีรายการ AP (APS) ของโครงการนี้</div>}
                        {p.apGroups.map((g, i) => {
                          const m = PR_META[g.status] || PR_META.none;
                          const vendors = [...new Set([...g.paidRows, ...g.outRows].map(r => r.vendor))].slice(0, 2).join(', ');
                          return (
                            <div key={i} style={{ borderTop: '1px solid var(--line-soft)' }}>
                              <div style={{ display: 'grid', gridTemplateColumns: '0.7fr 1fr 0.85fr 0.85fr 1.05fr', padding: '9px 12px', gap: 8, fontSize: 12, alignItems: 'start' }}>
                                <span><b>{g.key}</b></span>
                                <span style={{ fontSize: 11.5, color: g.unlocked ? '#15803d' : 'var(--ink-400)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={g.condLabel}>{g.condLabel === '—' ? '—' : ((g.unlocked ? '✓ ' : '✗ ') + g.condLabel)}</span>
                                <span style={{ fontFamily: "'IBM Plex Mono',monospace", textAlign: 'right', color: g.paidAmt > 0 ? 'var(--ink-800)' : 'var(--ink-300)' }}>{g.paidAmt > 0 ? prMoney(g.paidAmt) : '—'}</span>
                                <span style={{ fontFamily: "'IBM Plex Mono',monospace", textAlign: 'right', color: g.outAmt > 0 ? '#b45309' : 'var(--ink-300)' }}>{g.outAmt > 0 ? prMoney(g.outAmt) : '—'}</span>
                                <span style={{ textAlign: 'right' }}>
                                  <span style={prPill(m)}><span style={prDot(m)} />{m.label}</span>
                                  {g.status === 'planned' && canEdit && <div style={{ marginTop: 4 }}><button onClick={() => doUnplan(g)} style={{ padding: '3px 8px', borderRadius: 6, border: '1px solid var(--line)', background: '#fff', color: 'var(--ink-500)', fontSize: 10.5, cursor: 'pointer' }}>ยกเลิกแผน</button></div>}
                                  {canEdit && (g.status === 'ready' || g.status === 'overdue') && g.outRows.some(r => r.vchno) && (
                                    <button onClick={() => { setPlanForm({ payDate: all.todayISO, bankAc: '' }); setPlanTarget({ proj: p, group: g }); }}
                              ...
                              {vendors && <div style={{ padding: '0 12px 8px', fontSize: 10.5, color: 'var(--ink-400)' }}>ผู้รับเหมา: {vendors}{[...new Set([...g.paidRows, ...g.outRows].map(r => r.vendor))].length > 2 ? '…' : ''}{g.outRows.some(r => r.planned && r.plannedDate) ? ' · 📅 ' + fmtDate(g.outRows.find(r => r.plannedDate).plannedDate) : ''}</div>}
                            </div>
                          );
                        })}
                      </div>
